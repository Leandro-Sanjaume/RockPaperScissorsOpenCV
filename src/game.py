...
Model = load_model("models/rps3.h5")
...
def show_winner(user_socre, computer_score):    
    ...
    if user_score > computer_score:
        img = cv2.imread(giveImages()[0])
        
    elif user_score < computer_score:
        img = cv2.imread(giveImages()[1])
        
    else:
        img = cv2.imread(giveImages()[2])
...
def giveImages():
    if seriedad: 
        return ["images/playerWin.png", "images/playerLose.png","images/playerDraw.png"]
    else :
        return ["images/defaultWin.jpg","images/defaultLose.jpg", "images/defaultDraw.jpg"]
...
while True:
    ...
    if prob >= confidence_threshold:
        ...
        try:
            final_user_move = max(set(de), key = de.count)
        ...
        if final_user_move != "nothing" and hand_inside == False:
            ...
            computer_move_name = rigged_winner(final_user_move)
